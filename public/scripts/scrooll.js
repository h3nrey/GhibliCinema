...
const shortsListhorizontal = document.querySelectorAll(".cardsList")[3];

const shandleLeftArrow = () => {
    //let x = movieListhorizontal.style.marginLeft + Math.round(window.innerWidth / 2);
    // sX += Math.round(window.innerWidth / 3.7);
    sX += scroolXSize;
    if(sX > 0){
        sX = 0;
    }
    
    shortsListhorizontal.style.marginLeft = `${sX}px`;
}

var sX = 0;
const shandleRightArrow = () =>{
    //sX += -Math.round(window.innerWidth / 3.7);
    let scroolxMultiplier = -scroolXSize * 7;
    sX += -scroolXSize;

    if (sX < scroolxMultiplier) {

        sX = scroolxMultiplier
    }

    shortsListhorizontal.style.marginLeft = `${sX}px`;
}

const sArrowRight = document.querySelector('div#s-r-arrow');

sArrowRight.addEventListener('click', shandleRightArrow);

const sArrowLeft = document.querySelector('div#s-l-arrow');
sArrowLeft.addEventListener('click', shandleLeftArrow);